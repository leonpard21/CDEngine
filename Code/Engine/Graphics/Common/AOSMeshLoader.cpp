...
#endif
			pAOSMesh = CreateAOSMeshInternal(elements, &pData->_vertices[0], (uint32_t)pData->_vertices.size(),
				&pData->_indices[0], (uint32_t)pData->_indices.size(),
				&pData->_subMeshes[0], (uint32_t)pData->_subMeshes.size());
			return pAOSMesh;
		}
