...
#ifdef EAEENGINE_PLATFORM_GL
	const char* s_mainWindowClass_name = "Yang_Li's Main Window Class OpenGL Implementation";
#else 
	const char* s_mainWindowClass_name = "Yang_Li's Main Window Class D3D9 Implementation";
#endif
}
...
int CreateMainWindowAndReturnExitCodeWhenItCloses( const HINSTANCE i_thisInstanceOfTheProgram, const int i_initialWindowDisplayState )
{
	...
	if ( CreateMainWindow( i_thisInstanceOfTheProgram, i_initialWindowDisplayState ) )
	{
		...
		if (!EAE_Engine::Engine::EngineBegin(s_mainWindow))
		{
			EAE_Engine::UserOutput::Print("Something wrong happened in Engine Inilization.\n");
			// Clean up anything that was created/registered/initialized
			if (OnMainWindowClosed(i_thisInstanceOfTheProgram))
			{
				GameplayExit();
				EAE_Engine::Engine::EngineEnd();
				return -1;
			}
		}
		// Calculate how much of the "client area"
		// (not include the borders and title bar)
		{
			...
			float windowWdith = (float)(clientDimensions.right - clientDimensions.left);
			float windowHeight = (float)(clientDimensions.bottom - clientDimensions.top);
			if (!GameplayInit(windowWdith, windowHeight))
			{
				EAE_Engine::UserOutput::Print("Something wrong happened in Gameplay Inilization.\n");
				// Clean up anything that was created/registered/initialized
				if (OnMainWindowClosed(i_thisInstanceOfTheProgram))
				{
					EAE_Engine::Engine::EngineEnd();
...
HWND CreateMainWindowHandle( const HINSTANCE i_thisInstanceOfTheProgram, const int i_initialWindowDisplayState )
{
	...
	HWND mainWindow;
	{
		...
		const char* windowCaption = s_mainWindowClass_name;
		// The window's style
